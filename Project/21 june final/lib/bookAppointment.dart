...
///////////////
  ...
  onPressed: () async {
    ...
    if(cardnum.text.isEmpty ||cardname.text.isEmpty||cardcode.text.isEmpty||cardyear.text.isEmpty ){
      ...
      message="Payment Unsuccessful: no digits allowed in the name unless you are Elon Musk's son";
      _showDialog(context, message);
    }
  else  if(cardnum.text.length<16 || cardnum.text.length>19){
      message="Payment Unsuccessful: card number length should be 16-19 digits";
      _showDialog(context, message);
    }
   else if(cardcode.text.length!=3){
      message="Payment Unsuccessful: card code length should be 3 digits";
      _showDialog(context, message);
    }
    else if(cardyear.text.length!=4){
      message="Payment Unsuccessful: invalid year";
      _showDialog(context, message);
    }
    else if(int.parse(cardyear.text.toString()) < int.parse(_today.year.toString() )){
      message="Payment Unsuccessful: card expired";
      _showDialog(context, message);
    }
    else{
...
class BookAppointment extends State<bookAppointment> {
  ...
  final double tabBarHeight = 100;//100
  ...
  compareTime() async {
    ...
    else if (time.hour < od.hour || (time.hour==od.hour && time.minute < od.minute))
    {
      print("early");
  ...
  Widget build(BuildContext context) {
    ...
    _calendarCarouselNoHeader = CalendarCarousel<Event>(
      todayBorderColor: Color(0xff253976),
...
//      firstDayOfWeek: 4,
    ...
    return new Scaffold(
        ...
        appBar: AppBar(title:Text(('Select A Date')),backgroundColor: Colors.white.withOpacity(0.0)),

        body: Stack(
            children: <Widget>[
        new Container(
        decoration: new BoxDecoration(image: new DecorationImage(image: new AssetImage("assets/images/bookapt-100.jpg"), fit: BoxFit.fill)),
    ),
    SingleChildScrollView(
            ...
            children: <Widget>[
              ...
              Container(
                margin: EdgeInsets.symmetric(horizontal: 16.0),//16.0
                      ...
                      children: <Widget>[
                        ...
                        InkWell(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(color: Color(0xff8c4b83).withOpacity(0.6),borderRadius: BorderRadius.all(Radius.circular(20.0))),
                            child: TextFormField(
              ...
              Container(
                ...
                child: FlatButton(
                  ...
                  onPressed: () async{
                    ...
                    if ((_today2.year < today.year) ||( _today2.year==today.year && _today2.month<today.month) || ( _today2.year==today.year && _today2.month==today.month && _today2.day<today.day) ){
                      DialogBox d= new DialogBox();
                      d.showDialogInfo(context, "Cant Go back In Time!", "Oops! Looks like you're trying to go to the past. Select a day in the present or future.");
                    }
                    else {
                      ...
                      if (validTime == false)
                        ...
                        CollectionReference aptRef = firestoreInstance
                            .collection(
                            "Appointments");
                        List<String> items = [];
                        int price = 0;
                ...
                SlidingUpPanel(controller: panelController,
                    ...
                    color: Color(0xff8c4b83).withOpacity(0.6),
