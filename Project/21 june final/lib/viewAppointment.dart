...
class _viewAppointment extends State<viewAppointment>{
  Future<int> getAllSalonAppointment(salonName,date) async {
    ...
    firestoreInstance.collection("Appointments").get().then((querySnapshot){
      ...
      setState(() {
        ...
        final allData = querySnapshot.docs.map((doc) => doc.id + " " + doc.data()['salon'].toString()+" "+doc.data()['customer']+" "+doc.data()['datetime'].toDate().toString()+" Advance Payment: "+doc.data()['advancePayment'].toString()+" ,Services booked: "+doc.data()['services'].toString()).toList() ;
        Allappointments = allData;
        final allData2 = querySnapshot.docs.map((doc) => doc.data()['salon'].toString()+" :\ncustomer: "+doc.data()['customer']+""+doc.data()['datetime'].toDate().toString()+"\n Advance Payment: "+doc.data()['advancePayment'].toString()+" \nServices booked: "+doc.data()['services'].toString()).toList() ;
        Allappointments2 = allData2;
        print("listt");
        ...
        for (int i=0;i<Allappointments.length;i++) {
          ...
          if (Allappointments[i].split(" ")[1].toString() == salonName) {
            ...
            if (date.split(" ")[0] ==
              ...
              ShowAppointments.add(Allappointments2[i]);
  ...
  Future<void>initState() {
    ...
    for(int i=0;i<ShowAppointments.length;i++){

      print(date.split(" ")[0]);
      if(ShowAppointments[i].split(" ")[3]!=date.split(" ")[0]){
        ShowAppointments.removeAt(i);
      }
    }
                        ...
                        child: new Container(
                          ...
                          child: new ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int index) =>
                                Container(
                                  child: ListTile(
                                    focusColor: Color(0xff133579),
                                    hoverColor:Color(0xff133579),
