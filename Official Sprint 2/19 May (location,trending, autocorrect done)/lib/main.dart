...
Future getEmployees(salonname) async{
  firestoreInstance.collection("Salon").doc(salonname).collection("Employees").get().then((querySnapshot) {
    TotalEmployees = querySnapshot.docs.length;
    querySnapshot.docs.forEach((doc) {
      Employees.add(doc.id);
    });
  }).catchError((onError) {
    print("getCloudFirestoreEmployees: ERROR");
    print(onError);
        ...
        onPressed: () async {
          ...
          if (customer.exists &&
            ...
            Navigator.push(context,
                MaterialPageRoute(builder: (_) => SalonOwnerDashboard(email.text)));
          }
          else {
            await getSalons();
            for (var i = 0; i < TotalSalons; i++) {
              ...
              if (employee.exists &&
                ...
                foundEmp = 1;
                Navigator.push(context, MaterialPageRoute(
                    builder: (_) => employeeDashboard(email.text, Salons[i])));
              }
            }
          }
          if (foundEmp == 0 )
            showDialogInfo(context, "Incorrect Email or Password");
