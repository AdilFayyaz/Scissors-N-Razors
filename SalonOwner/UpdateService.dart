...
class UpdateService extends StatefulWidget{
  ...
  UpdateService(this.email, this.salonname);
  @override
  upService createState() => upService(this.email, this.salonname);
}
...
class upService extends State<UpdateService>{
  ...
  final String salonname;
  String Id;
  upService(this.email, this.salonname);
      ...
      appBar: AppBar(
        actions:[
          FlatButton(onPressed: (){
            ...
            {
              final docRef= salonRef.doc(name.text).update({
                'price': price.text
              });
            }
